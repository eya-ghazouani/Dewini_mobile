...
const Submit = async () => {
    ...
    if (!nom.trim() || nom.length<3) return updateError('Nom doit etre supérieur à 2 caractères!', setError)

        else if (!prenom.trim() || prenom.length<3) return updateError('Prénom doit etre supérieur à 2 caractères!!', setError)

        else if (!adresse.trim() || adresse.length<3) return updateError('Adresse doit etre supérieur à 2 caractères!', setError)

        else if (!tel.toString().trim() || tel.length<8 || tel.length>8) return updateError('Le numéro de téléphone doit etre 8 chiffres!', setError)

        else if((password!==null)&&(password.length <8)) {

         return updateError('Le mot de passe doit etre supèrieur à 8 caractères!', setError)
