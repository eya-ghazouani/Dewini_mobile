...
const Details_UserMedic = ({ route, navigation }) => {
    ...
    const [donnations, setDonnations] = useState([]);
...
const toggleDialog1 = () => {
    ...
    if ((!title)||(!qte)) return updateError('Tous les champs sont obligatoire!', setError)
        ...
        if(((selectedValueType!='Produit Paramedical'))
        ||((selectedValueType=='Medicament')))
        {
            if ((!selectedValueForme)) return updateError('La forme est obligatoire!', setError)
        ...
        formData.append("category", category);
            ...
            if ((!deadline)&&(!moment(date).format('DD/MM/YYYY'))) return updateError("La date d'expération est obligatoire!", setError)
                ...
                if(moment(date).isBefore(moment(today).add(30, 'days')))
                   {
...
<Card style={{width: '50%', height:'30%',marginLeft:90, marginTop:10}} >
            ...
            <View style={{ width: '100%', display: 'flex',  marginTop:10}} >
                ...
                <Text style={{fontSize: 20, fontWeight: '600', marginTop:20}} >
                    <Text style={{fontWeight:'bold'}}>Quanitité à donner : </Text>
                    {donn &&  
                        <>
                            {donn.qte_don} {forme} {forme ? <Text>(s)</Text> : null}
                        </>
                    }
                    </Text>
                <Text style={{fontSize: 20, fontWeight: '600', marginTop:20}}>
                <Text style={{fontWeight:'bold'}}>Type : </Text>{selectedValueType}
                </Text>
    ...
    { (((selectedValueType!='Produit Paramedical'))
       ||((selectedValueType=='Medicament'))) ?  
          <View style={{flexDirection:'row'}}>
               <Title style={{padding:5, margin:5,width: '30%'}}>Forme:</Title>
                    <View style={{}}>
                         <Picker
                            selectedValue={selectedValueForme}
                            style={{ height: 50, width: 250 }}
